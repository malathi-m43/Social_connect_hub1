...
def init_db():
    ...
    if admin_email and admin_password:
        ...
        if existing_admin:
            if existing_admin[1] == 0:
                cursor.execute(
                    'UPDATE users SET is_admin = 1, username = ? WHERE email = ?',
    ...
    if cursor.fetchone()[0] == 0:
        ...
        cursor.executemany(
            'INSERT INTO influencers (name, platform, followers, promotions, profile_url, collaboration_email, created_at) VALUES (?, ?, ?, ?, ?, ?, ?)',
            sample_influencers,
        )
...
def favorites(email):
    ...
    if request.method == 'POST':
        ...
        cursor.execute(
            'SELECT id FROM influencers WHERE LOWER(name) = ? AND LOWER(platform) = ?',
        ...
        cursor.execute(
            'SELECT id FROM favorites WHERE user_email = ? AND influencer_id = ?',
            (email, influencer['id']),
        )
        ...
        cursor.execute(
            'INSERT INTO favorites (user_email, influencer_id, created_at) VALUES (?, ?, ?)',
    ...
    cursor.execute(
        'SELECT i.* FROM influencers i JOIN favorites f ON f.influencer_id = i.id WHERE f.user_email = ? ORDER BY f.created_at DESC',
        (email,),
    )
    rows = cursor.fetchall()
...
def admin_stats():
    ...
    cursor.execute(
        'SELECT COUNT(*) AS count FROM influencers WHERE LOWER(platform) = ?',
        ('instagram',),
    )
